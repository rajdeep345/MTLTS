...
dfc = pd.read_pickle(
    "/content/drive/My Drive/BTP_Chandana_Vishnu/summarization/Data/dfc_0625.pkl")
dfg = pd.read_pickle(
    "/content/drive/My Drive/BTP_Chandana_Vishnu/summarization/Data/dfg_08.pkl")
dfo = pd.read_pickle(
    "/content/drive/My Drive/BTP_Chandana_Vishnu/summarization/Data/dfo_069.pkl")
dfs = pd.read_pickle(
    "/content/drive/My Drive/BTP_Chandana_Vishnu/summarization/Data/dfs_0675.pkl")
...
def generate_summary(numwords, dfsum):
    ...
    if len(dfpred1) != 0:
        dfpred1.sort_values(by=['summ_pred_prob'],
                            ascending=False, inplace=True)
        dfpred1.drop_duplicates(
            subset=['Clean_Tweet'], keep='first', inplace=True)
    ...
    if summary_length < numwords:
        ...
        if len(dfpred0) != 0:
            dfpred0.sort_values(by=['summ_pred_prob'],
                                ascending=True, inplace=True)
            dfpred0.drop_duplicates(
                subset=['Clean_Tweet'], keep='first', inplace=True)
            dfpred0["Num_words"] = dfpred0.apply(
                lambda x: get_numwords(x), axis=1)
            l0 = len(dfpred0)
...
class Hierarchial_MTL(torch.nn.Module):
    ...
    def __init__(self, model_name, trainable_layers, in_features, out_features, classifier_dropout, mode, tweetconfig=tweetconfig):
        ...
        if model_name == 'BERT':
            ...
            self.bert = RobertaModel.from_pretrained(
                "/content/BERTweet_base_transformers/model.bin", config=tweetconfig)
        ...
        self.U_iou = torch.nn.Linear(
            self.out_features, 3 * self.out_features, bias=False)
        ...
        if USE_SITU == 'y':
            self.veri_fc = torch.nn.Linear(self.out_features+1, 2)
        else:
        ...
        # Summarizer layers.
        if USE_SITU == 'y':
            self.summ_fc1 = torch.nn.Linear(
                self.in_features + 130 + 1, self.out_features)
        else:
            self.summ_fc1 = torch.nn.Linear(
                self.in_features + 130, self.out_features)
    ...
    def forward(self, features, attentions, old_features, node_order, adjacency_list, edge_order, root_node, sit_tags, trainable_part=None):
        ...
        bert_outputs = self.BERT_model(
            input_ids=features, attention_mask=attentions)
        ...
        if self.mode == "cls":
            ...
            input_mask_expanded = attentions.unsqueeze(
        ...
        if trainable_part == 'verifier':
            ...
            h_root, veri_logits_out = self.verify_forward(
                h, c, output_vectors, node_order, adjacency_list, edge_order, root_node)
    ...
    def verify_forward(self, h, c, output_vectors, node_order, adjacency_list, edge_order, root_node):
        # Verification
        for n in range(node_order.max() + 1):
            self._run_lstm(n, h, c, output_vectors, node_order,
    ...
    def summarize_forward(self, h_root, veri_logits_out, output_vectors, root_node):
        # Summarization
        # summ_in = output_vectors[root_node,:]
        summ_in = torch.cat([output_vectors[root_node, :],
                             h_root, veri_logits_out], axis=1)
        if USE_SITU == 'y':
            sit_tags = sit_tags.view(-1, 1)
            summ_in = torch.cat([summ_in, sit_tags], axis=1)
        summ_logits1 = self.summ_fc1(summ_in)
    ...
    def _run_lstm(self, iteration, h, c, features, node_order, adjacency_list, edge_order):
        ...
        if iteration == 0:
            ...
            _, child_counts = torch.unique_consecutive(
                parent_indexes, return_counts=True)
...
class Theta():
    ...
    def get_successive_regualization_term(theta1, theta2):
        assert len(theta1.layers) == len(theta2.layers)

        difference = 0
        for theta1_layer, theta2_layer in zip(theta1.layers, theta2.layers):
            difference += torch.sum((theta1_layer.weight -
                                     theta2_layer.weight)**2)
            if theta1_layer.bias is not None and theta1_layer.bias is not None:
                difference += torch.sum((theta1_layer.bias -
                                         theta2_layer.bias)**2)
        return difference
...
if MODEL_NAME == 'BERT':
    ...
    tree_path = '/content/drive/My Drive/BTP_Chandana_Vishnu/PT_PHEME5_FeatBERTWEET40_Depth5_maxR5_MTL_Final/'

files = ['charliehebdo.txt', 'germanwings-crash.txt',
         'ottawashooting.txt', 'sydneysiege.txt']
...
for filename in files:
    ...
    input_file.close()
    print(
        f'{filename} Training Set Size: {len(tree_li[filename])}, Validation Set Size: {len(val_li[filename])}, Total: {len(tree_li[filename]) + len(val_li[filename])}')
...
for test_file in files:
    ...
    print(
        f'Total non-summary-tweets: {summ_label_dist[0]}, Total summry-tweets: {summ_label_dist[1]}')
    weight_vec[test_file] = torch.tensor(
        compute_class_weight('balanced', numpy.unique(y), y)).to(device)
    summ_weight_vec[test_file] = torch.tensor(compute_class_weight(
        'balanced', numpy.unique(summ_y), summ_y)).to(device)
    pos_weight = label_dist[0] / label_dist[1]
    pos_weight_vec[test_file] = torch.tensor(
        [pos_weight], dtype=torch.float32).to(device)
    summ_pos_weight = summ_label_dist[0] / summ_label_dist[1]
    summ_pos_weight_vec[test_file] = torch.tensor(
        [summ_pos_weight], dtype=torch.float32).to(device)
    print(
        f'Test File: {test_file}, Verification Weight Vector: {weight_vec[test_file]}')
    print(
        f'Test File: {test_file}, Verification Pos Weight Vector: {pos_weight_vec[test_file]}')
    print(
        f'Test File: {test_file}, Summary Weight Vector: {summ_weight_vec[test_file]}')
    print(
        f'Test File: {test_file}, Summary Pos Weight Vector: {summ_pos_weight_vec[test_file]}')
...
def train_loop(model: Hierarchial_MTL, tree_batch, test_file, theta_dash=None, mode="train", module=None):
    ...
    if mode == 'test':
        ...
        g_labels_tensor = torch.tensor(
            g_labels.clone().detach()).type_as(veri_logits_out).to(device)
        ...
        summ_gt_labels_tensor = torch.tensor(
            summ_gt_labels.clone().detach()).type_as(summ_out).to(device)
        summ_logits = summ_out.detach().cpu()
        pred_v_summ, pred_labels_summ = torch.max(
            F.softmax(summ_logits, dim=1), 1)
    ...
    if module == 'summarizer':
        ...
        loss = loss_function_summ(
            summ_out, tree_batch['summ_gt'].long().to(device))
    ...
    if module == 'summarizer':
        theta = Theta(model.W_iou, model.U_iou,
                      model.W_f, model.U_f, model.veri_fc)
        # print(loss.grad_fn)
        loss += DELTA * \
            Theta.get_successive_regualization_term(theta, theta_dash)
        # print(loss.grad_fn)
...
def get_data(x):
    orig_tweet_text = dftest.loc[dftest.Tweet_ID ==
                                 int(x['Tweet_ID']), 'Orig_Tweet'].values[0]
    clean_tweet_text = dftest.loc[dftest.Tweet_ID == int(
        x['Tweet_ID']), 'Clean_Tweet'].values[0]
    sit_label = dftest.loc[dftest.Tweet_ID == int(
        x['Tweet_ID']), 'Situational'].values[0]
    orig_summ_gt = dftest.loc[dftest.Tweet_ID ==
                              int(x['Tweet_ID']), 'Summary_gt'].values[0]
    new_summ_gt = dftest.loc[dftest.Tweet_ID == int(
        x['Tweet_ID']), 'New_Summary_gt'].values[0]
    r1nr0 = dftest.loc[dftest.Tweet_ID == int(
        x['Tweet_ID']), 'R1NR0'].values[0]
...
for lr in lr_list:
    ...
    for test in files:
        ...
        # Name of model.
        name = path + "stl_verification_" + TEST_FILE + "_" + \
            str(IN_FEATURES) + "_feat" + MODEL_NAME + ".pt"
        tweetconfig.output_attentions = False
        tweetconfig.output_hidden_states = False
        model = Hierarchial_MTL(MODEL_NAME, TRAINABLE_LAYERS, IN_FEATURES,
                                OUT_FEATURES, CLASSIFIER_DROPOUT, mode="cls", tweetconfig=tweetconfig)
        model.cuda(gpu_id)
        tweetconfig2 = copy.deepcopy(tweetconfig)
        tweetconfig2.output_attentions = True
        tweetconfig2.output_hidden_states = True
        test_model = Hierarchial_MTL(MODEL_NAME, TRAINABLE_LAYERS, IN_FEATURES,
                                     OUT_FEATURES, CLASSIFIER_DROPOUT, mode="cls", tweetconfig=tweetconfig2)
        ...
        for i in range(NUM_ITERATIONS):
            ...
            for tree_batch in data_gen:
                ...
                loss, p_labels, p_probs, g_labels = train_loop(
                    model, tree_batch, test_file, mode="train", module="verifier")
                ground_labels.extend(g_labels)
                predicted_labels.extend(p_labels)
                j = j+1
            ...
            theta_dash = Theta(model.W_iou, model.U_iou,
                               model.W_f, model.U_f, model.veri_fc)
            summ_ground_labels = []
            summ_predicted_labels = []
            ...
            err_count = 0
            for tree_batch in data_gen:
                loss, p_summ_labels, p_summ_probs, summ_gt_labels = train_loop(
                    model, tree_batch, test_file, theta_dash=theta_dash, mode="train", module="summarizer")
                summ_ground_labels.extend(summ_gt_labels)
                summ_predicted_labels.extend(p_summ_labels)
                summ_train_avg_loss += loss
            summ_train_acc = accuracy_score(
                summ_ground_labels, summ_predicted_labels)
            ...
            with torch.no_grad():
                for batch in val_gen:
                    loss, p_labels, p_probs, g_labels, p_summ_labels, p_summ_probs, summ_gt_labels = train_loop(
                        model, batch, test_file, theta_dash=theta_dash, mode="eval")
                    ...
                    val_j += 1
                    val_avg_loss += loss
            veri_val_acc = accuracy_score(
                val_ground_labels, val_predicted_labels)
            veri_val_f1 = f1_score(val_ground_labels, val_predicted_labels)
            summ_val_acc = accuracy_score(
                val_summ_ground_labels, val_summ_predicted_labels)
            summ_val_f1 = f1_score(
                val_summ_ground_labels, val_summ_predicted_labels)
            val_avg_loss /= val_j
            ...
            if ((i+1) % 5 == 0 and i > 0):
                ...
                with torch.no_grad():
                    for test in test_trees:
                        ...
                        pred_v, pred_label = torch.max(
                            F.softmax(pred_logits, dim=1), 1)
                        ...
                        summ_logits = summ_out.detach().cpu()
                        pred_v_summ, pred_label_summ = torch.max(
                            F.softmax(summ_logits, dim=1), 1)
                ...
                print(classification_report(
                    veri_ground, veri_predicted, digits=5))
                print("\n\n")
                print('confusion matrix ', confusion_matrix(
                    veri_ground, veri_predicted))
                ...
                dfsum[["Orig_Tweet", "Clean_Tweet", "Situational", "Summary_gt",
                       "New_Summary_gt", "R1NR0"]] = dfsum.apply(lambda x: get_data(x), axis=1)
                ...
                dfsum.sort_values(
                    by=['summ_pred', 'summ_pred_prob'], inplace=True, ascending=False)
                dfsum = dfsum.reset_index(drop=True)

                groundtruth_summary_original = ""
                glist_original = dfsum[dfsum['Summary_gt']
                                       == 1]['Orig_Tweet'].values
                groundtruth_summary_original = " .\n".join(glist_original)

                groundtruth_summary_cleaned = ""
                glist_cleaned = dfsum[dfsum['Summary_gt']
                                      == 1]['Clean_Tweet'].values
                groundtruth_summary_cleaned = " .\n".join(glist_cleaned)

                if L2_REGULARIZER == 'n':
                    dfsum.to_pickle("MTL_" + TEST_FILE + "_" + MODEL_NAME + "_" + str(IN_FEATURES) + "_SIT_" + str(
                        USE_SITU) + "_L2_n_LR_" + str(lr) + "_LAM1_" + str(LAMBDA1) + "_LAM2_" + str(LAMBDA2) + ".pkl")
                else:
                    dfsum.to_pickle("MTL_" + TEST_FILE + "_" + MODEL_NAME + "_" + str(IN_FEATURES) + "_SIT_" + str(USE_SITU) +
                                    "_L2_y_" + str(WEIGHT_DECAY) + "_LR_" + str(lr) + "_LAM1_" + str(LAMBDA1) + "_LAM2_" + str(LAMBDA2) + ".pkl")

                summ_orig, summ_clean, veri_prop, summary_length = generate_summary(
                    250, dfsum)
                ...
                if L2_REGULARIZER == 'y':
                    summary_filepath = "STL_" + MODEL_NAME + "_" + str(IN_FEATURES) + "_" + str(
                        lr) + "_L2y_" + str(WEIGHT_DECAY) + '_' + TEST_FILE + "_summary.txt"
                else:
                    summary_filepath = "STL_" + MODEL_NAME + "_" + \
                        str(IN_FEATURES) + "_" + str(lr) + \
                        "_L2n_" + TEST_FILE + "_summary.txt"
                        ...
                        for metric, results in sorted(scores.items(), key=lambda x: x[0]):
                            if not apply_avg and not apply_best:  # value is a type of list as we evaluate each summary vs each reference
                                ...
                                print(prepare_results(
                                    results['p'], results['r'], results['f']))
                        print()

                print("\n\nFor Original Summary of length 250..")
                if summ_orig.strip() != "":
                    os.chdir("twitie-tagger")
                    return_code = subprocess.call("java -jar twitie_tag.jar models/gate-EN-twitter.model '../" +
                                                  MODEL_NAME + "_test_orig_250.txt' > '../" + MODEL_NAME + "_output_orig_250.txt'", shell=True)
                    os.chdir("..")
                ...
                if summ_clean.strip() != "":
                    os.chdir("twitie-tagger")
                    return_code = subprocess.call("java -jar twitie_tag.jar models/gate-EN-twitter.model '../" +
                                                  MODEL_NAME + "_test_clean_250.txt' > '../" + MODEL_NAME + "_output_clean_250.txt'", shell=True)
                    os.chdir("..")
