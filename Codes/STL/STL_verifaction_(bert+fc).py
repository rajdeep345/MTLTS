...
class TreeLSTM(torch.nn.Module):
	...
	def __init__(self, model_name, trainable_layers, in_features, out_features, classifier_dropout, mode):
		...
		if model_name == 'BERT':
			...
			config = RobertaConfig.from_pretrained("./BERTweet_base_transformers/config.json")
			self.BERT_model = RobertaModel.from_pretrained("./BERTweet_base_transformers/model.bin",config=config)
...
if MODEL_NAME == 'BERT':
	...
	tree_path = './drive/BERTWEET/PT_PHEME5_FeatBERTweet40_Depth5_maxR5_MTL_modelSIT/'
elif MODEL_NAME == 'ROBERTA':
