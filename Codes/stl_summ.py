...
d = {}
with open("MTLVS/data/features/all_tweets_posteriors.txt") as file:
	for line in file:
		line = line.split("\t")
		d[int(line[1])] = eval(line[2])

# Appending the situational feature to make it a 41 dimensional vector
def get_features(x):
	features = []
	situ = float(x["Situ1_NonSitu0_Oth2"])
	if IN_FEATURES == 769:		
...
dfc = pd.read_pickle("MTLVS/data/features/summary_dataframes/dfc_0.57.pkl")
dfg = pd.read_pickle("MTLVS/data/features/summary_dataframes/dfg_0.72.pkl")
dfo = pd.read_pickle("MTLVS/data/features/summary_dataframes/dfo_0.6.pkl")
dfs = pd.read_pickle("MTLVS/data/features/summary_dataframes/dfs_0.6.pkl")
...
class BertForSequenceClassificationModified(BertPreTrainedModel):
	...
	def forward(
		...
		if MODEL_NAME == 'BERT':
			...
			# pooled_output = self.dropout1(cls)
		elif MODEL_NAME == 'ROBERTA':
			sequence_output = outputs[0]
			cls = sequence_output[:, 0]
...
for lr in lr_list:
	...
	for place, dflist in place_summary.items():
		...
		path = "./Models/"
		name = path + "stl_summ_feat_" + MODEL_NAME + ".pt"

		# Varies wrt to the training-test combination
		y =list(dft['New_Summ_gt_Clean'].values)
		y = np.array(y)
		class_weights = compute_class_weight('balanced', np.unique(y),y)
		
		sentences = dft.Clean_Tweet
		labels = dft.New_Summ_gt_Clean
		...
		sentences = dftest.Clean_Tweet.values
		labels = dftest.New_Summ_gt_Clean.values
		...
		# For every sentence...
		for sent in sentences:
			tokenslist.append(tokenizer.tokenize(sent))
			encoded_dict = tokenizer.encode_plus(
								sent,							# Sentence to encode.
								add_special_tokens = True,		# Add '[CLS]' and '[SEP]'
								max_length = 32,				# Pad & truncate all sentences.
		...
		dfsum = pd.DataFrame({'Tweet_ID': dfp.Tweet_ID,
							...
							'Norm_Tweet': dfp.Norm_Tweet,
							'Summary_gt': dfp["Summary_gt"],
							'New_Summary_gt': dfp.New_Summ_gt_Clean,
							'pred': flat_predictions, 
							'Situational': dfp.Situ1_NonSitu0_Oth2,
							'R1NR0': dfp.R1NR0,
							'False0_True1_Unveri2_NR3_Rep4': dfp.False0_True1_Unveri2_NR3_Rep4,
							"Tokens": tokenslist,
		...
		summ_orig, summ_clean, veri_prop, modified_veri_prop, summary_length, summary_length_orig, summary_length_clean, count, total_verified, later_verified = generate_summary(250, dfsum)
		...
		with open(place + "_test_orig_250.txt",'w') as f:
			f.write(summ_orig)

		with open(place + "_test_clean_250.txt",'w') as f:
			f.write(summ_clean)
		...
		if summ_orig.strip() != "":
			...
			return_code = subprocess.call("/usr/lib/jvm/java-8-openjdk-amd64/jre/bin/java -jar twitie_tag.jar models/gate-EN-twitter.model '../" + place + "_test_orig_250.txt' > '../" + place + "_output_orig_250.txt'", shell=True)
			# return_code = subprocess.call("java -jar twitie_tag.jar models/gate-EN-twitter.model '../" + place + "_test_orig_250.txt' > '../" + place + "_output_orig_250.txt'", shell=True)
			os.chdir("..")

			num_words = 0
			content_words =0
			with open(place + "_output_orig_250.txt", 'r') as f:
				for line in f:
					words = line.split()
					for word in words:
						l = word.split("_")
		...
		if summ_clean.strip() != "":
			os.chdir("twitie-tagger")
			return_code = subprocess.call("/usr/lib/jvm/java-8-openjdk-amd64/jre/bin/java -jar twitie_tag.jar models/gate-EN-twitter.model '../" + place + "_test_clean_250.txt' > '../" + place + "_output_clean_250.txt'", shell=True)
			# return_code = subprocess.call("java -jar twitie_tag.jar models/gate-EN-twitter.model '../" + place + "_test_clean_250.txt' > '../" + place + "_output_clean_250.txt'", shell=True)
			os.chdir("..")

			num_words = 0
			content_words =0
			with open(place + "_output_clean_250.txt", 'r') as f:
				for line in f:
					words = line.split()
					for word in words:
						l = word.split("_")
