...
class Hierarchial_MTL(torch.nn.Module):
	...
	def __init__(self, model_name, trainable_layers, in_features, out_features, classifier_dropout, mode):
		...
		# Summarizer layers.
		if USE_SITU == 'y':
			self.summ_fc1 = torch.nn.Linear(self.in_features + 130 +1, self.out_features)
		else:
...
def train_loop(model:Hierarchial_MTL, tree_batch, test_file, theta_dash=None, mode="train", module=None):
	...
	if module == 'summarizer':
		...
		loss = loss_function_summ(summ_out, tree_batch['summ_gt'].long().to(device))
	...
	if module == 'summarizer':
		...
		# print(loss.grad_fn)
		loss += DELTA*Theta.get_successive_regualization_term(theta, theta_dash)
		# print(loss.grad_fn)
...
for lr in lr_list:
	...
	for test in files:
		...
		model = Hierarchial_MTL(MODEL_NAME, TRAINABLE_LAYERS, IN_FEATURES, OUT_FEATURES, CLASSIFIER_DROPOUT, mode="cls")
		model.cuda(gpu_id)
		test_model = Hierarchial_MTL(MODEL_NAME, TRAINABLE_LAYERS, IN_FEATURES, OUT_FEATURES, CLASSIFIER_DROPOUT, mode="cls")
		...
		for i in range(NUM_ITERATIONS):
			...
			if ((i+1) % 5 == 0 and i > 0):
				...
				dfsum[["Orig_Tweet", "Clean_Tweet", "Situational", "Summary_gt", "New_Summary_gt", "R1NR0"]] = dfsum.apply(lambda x : get_data(x), axis= 1)
