...
tweetconfig = RobertaConfig.from_pretrained("/content/BERTweet_base_transformers/config.json")
...
if __name__ == "__main__":
	...
	lr_list = [1e-6, 2e-6, 5e-6, 1e-5, 2e-5]
...
# Model SIT Labels
dfc = pd.read_pickle("/content/drive/My Drive/BTP_Chandana_Vishnu/dfc_0.57.pkl")
dfg = pd.read_pickle("/content/drive/My Drive/BTP_Chandana_Vishnu/dfg_0.72.pkl")
dfo = pd.read_pickle("/content/drive/My Drive/BTP_Chandana_Vishnu/dfo_0.6.pkl")
dfs = pd.read_pickle("/content/drive/My Drive/BTP_Chandana_Vishnu/dfs_0.6.pkl")
...
class Theta():
	...
	def get_successive_regualization_term(theta1, theta2):
		...
		for theta1_layer, theta2_layer in zip(theta1.layers, theta2.layers):
			difference += torch.sum((theta1_layer.weight - theta2_layer.weight)**2) 
			if theta1_layer.bias is not None and theta1_layer.bias is not None:
				difference += torch.sum((theta1_layer.bias - theta2_layer.bias)**2)
		return difference
...
def train_loop(model:Hierarchial_MTL, tree_batch, test_file, theta_dash=None, mode="train", module=None):
	...
	if mode == 'test':
		...
		g_labels_tensor = torch.tensor(g_labels.clone().detach()).type_as(veri_logits_out).to(device)
		...
		summ_gt_labels_tensor = torch.tensor(summ_gt_labels.clone().detach()).type_as(summ_out).to(device)
...
def get_data(x):
	...
	return pd.Series([orig_tweet_text, clean_tweet_text, orig_summ_gt, new_summ_gt, r1nr0])
	...
	for test in files:
		...
		for i in range(NUM_ITERATIONS):
			...
			if (i+1) % 5 == 0 and i > 0:
				...
				# dfsum.to_pickle('/content/dfsum.pkl')
				dfsum[["Orig_Tweet", "Clean_Tweet", "Summary_gt", "New_Summary_gt", "R1NR0"]] = dfsum.apply(lambda x : get_data(x), axis=1)
				...
				# if L2_REGULARIZER == 'n':
				# 	dfsum.to_pickle("HMTL_" + TEST_FILE + "_"  + MODEL_NAME + "_" + str(IN_FEATURES) + "_SIT_" + str(USE_SITU) + "_L2_n_LR_" + str(lr) + "_LAM1_" + str(LAMBDA1) + "_LAM2_" + str(LAMBDA2) + ".pkl")
				# else:
				# 	dfsum.to_pickle("HMTL_" + TEST_FILE + "_"  + MODEL_NAME + "_" + str(IN_FEATURES) + "_SIT_" + str(USE_SITU) + "_L2_y_" + str(WEIGHT_DECAY) + "_LR_" + str(lr) + "_LAM1_" + str(LAMBDA1) + "_LAM2_" + str(LAMBDA2) + ".pkl")
