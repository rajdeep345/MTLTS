...
for test_file in files:
	...
	print(f'Class Weight Vector: {weight_vec[test_file]}')
	print(f'Positive Class Weight Vector: {pos_weight_vec[test_file]}')
...
def train(tree_batch, test_file, mode="train"):
	...
	return loss, pred_labels, g_labels
	...
	for test in files:
		...
		for i in range(NUM_ITERATIONS):
			...
			for tree_batch in data_gen:
				loss, p_labels, g_labels = train(tree_batch, test_file, "train")
			...
			with torch.no_grad():
				for batch in val_gen:
					loss, p_labels, g_labels = train(batch, test_file, "eval")
			...
			val_f1 = f1_score(val_ground_labels, val_predicted_labels, average='macro')
			val_avg_loss /= val_j
		...
		with torch.no_grad():
			for test in test_trees:
				try:
					h_test, h_test_root, c, att = model(
							test['f'].to(device),
							test['a'].to(device),
							test['k'].to(device),
